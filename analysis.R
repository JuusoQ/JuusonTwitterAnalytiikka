...
top <- subset(pvm, user %in% all_time_top$user)
...
all_time_wsj <- ggplot(data=top, aes(x=date, y=count, fill=user)) + geom_line(stat="identity") + facet_wrap(~user) + xlab("Päivämäärä") + ylab("Tweettejä") + scale_x_date(breaks = "1 year") + theme_wsj()+ scale_fill_wsj()+ theme(legend.position="none")+theme(axis.text.x = element_text(angle = 45, hjust = 1))

## time series plot by date
by_date2 <- pvm %>% group_by(date) %>% summarise(by_date = sum(count))
twiitit.ts <- ggplot(data=by_date2, aes(x=date,y=by_date)) + geom_line(stat="identity")+ theme_wsj() + xlab("Päivämäärä") + scale_x_date(breaks="6 months")+theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
